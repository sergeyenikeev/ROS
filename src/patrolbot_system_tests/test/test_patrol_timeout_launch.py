...
def generate_test_description():
    ...
    return launch.LaunchDescription([
        Node(
            ...
            output='screen',
            parameters=[{'result_sequence': 'timeout'}],
        ),
        Node(
...
class TestPatrolTimeoutLaunch(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        rclpy.init()
        cls.node = rclpy.create_node('test_patrol_timeout_client')
    ...
    def test_patrol_fails_on_timeout(self):
        statuses = []
        ...
        while time.time() < deadline:
            ...
            if statuses and statuses[-1].state == 'FAILED':
                self.assertIn('таймаут', statuses[-1].last_error.lower())
                return

        self.fail('Миссия не перешла в состояние FAILED после таймаута')
